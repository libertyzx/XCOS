...
**修 10 项复核问题**(逐项回代码复核后 9 项成立), **无需改用户代码**; API 与数据结构未变(`sizeof(TCB)`/`sizeof(XCOS_t)` 仍 **44 / 44**)。

- **A19 跨回绕延时的任务可能再也不醒**: `PrevTick` 改为**每轮更新**(以前只在"检测到回绕/时间表到点"时更新 ⇒ 溢出表整表搬移被跳过, 任务永不唤醒);
- **B5 在任务里调自检必然误报 `TASKNUM`**: 运行中的任务不在任何表内 ⇒ 现允许"恰好少 1"(其余差值照报);
- **C5 `XC_Tick_t` 不再可配**: 固定 `uint32_t`(定义移到类型层), 旧宏 `XC_CFG_TICK_TYPE` 与编译期断言删除(64 位会截断延时 / 溢出表永不排空 / Tick 读撕裂);
- **A22 嵌套配置落点对齐**: `Reg` 清空 / `RegExt` **先注册再写帧栈**(原顺序会被静默抹掉) / 分步注册顺序 `SetEntry → SetCorStack → Add`;
- **A20/A21 两个小加固**: 空闲回调的 Tick 快照入锁(与表内取值一次观察); 通知数据 `pNotifyData` 加 `volatile`;
- **文档纠错**: `Suspend` 的错误 `@retval XC_CONTINUE`、`Sch_Init` 的"初始锁定"范围、`IsNotifyTimeout` 判据、`TCB 须静态/零初始化`、"一行只写一个协程控制宏"。

**影响与验证**: 功能 **PASS 20 / FAIL 0 / SKIP 3**(23 条, 含 3 条新增用例 `A19-跨回绕溢出表`/`B5-任务内自检`/`A22-嵌套注册`);
镜像 `default`/`stats`/`diag` = **8804 / 8960 / 9928**(**+36 / +40 / +60 B**, **RAM 0**); 让出往返 **1285 / 1286**(+4 周期); 内核 0 告警。

> 逐项证据与取舍见 `Docs/问题跟踪/`(清单 **20260917** 批 + 修正方案 `20260917_复核十项修正`), 数值口径见 `Tests/baseline.md`。
...
**新增可选开关 `XC_CFG_COR_NESTING`（默认开启）**：协程嵌套（`XC_Cor_Call` + 调度器"同轮切父"）可裁剪——关闭后
`sizeof(TCB)` **44 → 36 Byte**（**每任务 −8 B RAM**，100 任务 −800 B）、对象级 `XC_Sch.o`/`XC_Task.o` 各 **−28 / −144 B**，
嵌套 API 不编译（误用即编译报错）；基本协程能力不变；并新增编译期断言 `XC_CFG_COR_NESTING ∈ {0, 1}`（写错即编译报错）。
见 `Docs/XC_Config.md`「XC_CFG_COR_NESTING」。

**Tick 档位选型指南（改注释与文档，无行为变化）**：`XC_CFG_TICKS_PER_SEC` 的头文件注释与 `Docs/XC_Config.md` 补全"取值规则 / **回绕周期 = 2³²/f** / 推荐档位表（ms 档 16 个合法值、µs 档推荐值）/ 选档三步 / 非因子频率误差示例"。三条使用者要知道的边界：
- **回绕周期**（同时是 `XC_Time_SecToTicks()` 的秒数上限）：1 kHz ≈49.7 天、10 kHz ≈4.97 天、100 kHz ≈11.9 小时 ⇒ 要求"≥1 天"就选 `f ≤ ≈50 kHz`；
- **µs 档的毫秒入口有 ≈71.6 分钟上限**（`MsToTicks` 的 `ms × 1000` 是 32 位中间量 ⇒ 与频率无关；ms 档约 49.7 天）⇒ 更长的延时请用 `SecToTicks` / `DelayTick` 或分段；
- **µs 档 `TicksToMs()` 可能得 0**（1 tick < 1 ms 的精度下限）⇒ 需要 µs 精度请用 `TicksToUs()`。

**表格排版纪律与自检工具**：代码编辑器**不渲染**注释里的 Markdown ⇒ 表格必须在**源码里**就是齐的。
- 新增 **`Tests/check_tables.ps1`**：按"**ASCII = 1 列 / 中文·全角 = 2 列**"逐行算显示宽度，同一张表各行宽度不等即 `FAIL`（本仓 351 张表当前全部对齐；`-Verbose` 可逐块列出）；
- **对齐纪律已固化**：写进 `Docs/Skill/XCOS_Dev_Agent_Skill.md` §三 第 6 条（宽度口径 + 分隔行补满列宽）、§十二 陷阱 #24、§十三 命令速查；本轮共补齐 **25 个文件 / 351 张表**，并修掉 4 处"单元格内未转义 `|`"（会被 Markdown 切开一列）。

**影响与验证**：功能 **PASS 17 / FAIL 0 / SKIP 3**（20 条用例，含新增 `A1-嵌套关闭`：同一用例在 `-DXC_CFG_COR_NESTING=0` 档复跑）；体积 / 周期 / 让出往返 基线 **delta 全 0**；表格对齐自检 **PASS**（351 张表）；文档自检：坏链 0 / 围栏配对 0。
