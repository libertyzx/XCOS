...
- 框架核心修复(详见对应修正方案文档):
    ...
    - **修复"任务运行期挂起/移除就绪表后继任务导致调度游标失效"**
        ...
        - 验证:挂起后继不再被调度、6 项调度综合测试、嵌套/锁/状态回归全部通过;详见 `Docs/修正方案/20260821_调度器游标失效/修正方案.md`;
    ...
    - **修复"父层 XC_Cor_Call 让出后 TaskState 保持 RUN / 阻塞表僵尸"状态一致性缺陷**:
        ...
        - 验证:复现测试 Call 让出后状态 RUN(1)→READY(2);5 场景回归(延时/通知/挂起恢复/嵌套/顶层 DONE)与原内核输出一致;armcc(ANSI/GNU)/armclang 0 警告,UV4 完整工程构建 0 错误 0 警告;详见 `Docs/修正方案/20260821_调度器游标失效/修正方案.md` 第 13 章;
    - **已知缺陷(不修复,用户决策,已注释记录)**:`EventSched` 的 `WAKEUP_NOTIFY` 未排除挂起任务——等待通知(`WAIT`)中收到异步通知(中断锁窗口 SendNotify)后被挂起的任务,会被事件调度误唤醒,破坏"挂起只能 Resume 恢复"语义;已在宏注释中记录"使用注意",需用户保证"勿对等待通知中且可能收到异步通知的任务挂起";完整修复需配套 `XC_Task_Resume` 补消费通知;详见 `Docs/缺陷记录/20260824_事件调度误唤醒挂起任务.md`;
...
- 测试与开发工具:
    ...
    - **新增 AI Agent 开发技能 `Docs/XCOS_Agent_Skill.md`**:含框架概述/编程模型/API 速查/标准代码模式/约定与陷阱/文件索引/已知缺陷,示例代码经 gcc 链接真实内核编译验证(`-Wall -Wextra` 0 错误);
    - **README.md 文档入口完善**:新增"框架概览"与"Agent 开发技能"引用,目录树补齐 `修正方案/`/`缺陷记录/`/`XCOS_Agent_Skill.md`;
