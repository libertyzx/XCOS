...
6. **表格必须"源码对齐"**：**代码注释里必须**（编辑器不渲染注释 ⇒ 表格在**源码里**就得是齐的）；`*.md` 里单元格极长、主要供渲染阅读的表可不强制（如本文档的陷阱清单）。
   - 宽度按**显示列数**算：ASCII（`0x20~0x7E`）计 **1 列**，其余字符（中文 / 全角标点 / 箭头 / 圈号 / emoji）一律计 **2 列**（即"2 个英文 = 1 个中文"）；
   - 每列宽度 = 该列**最宽单元格**（`| --- |` 分隔行的横线**也补满该列宽度**，与 `XC_List.c` 既有样式一致），单元格左对齐补空格；
   - 表内单位/符号尽量写 ASCII（`us`、`~`、`->`、`<=`），避免 `µ ≈ ⇒ ① ⚠️` 这类**宽窄歧义**字符把表挤歪；
   - **自检**：`powershell -File Tests/check_tables.ps1`（同一张表**每行显示宽度必须完全相等**；`-Verbose` 逐块列出宽度）。
   - 已全量对齐：`Code/Inc/{XC_Config,XC_Diag,XC_Err}.h`、`Code/Src/XC_List.c`、`Docs/XC_Config.md`、`Tests/baseline.md`、`Docs/问题跟踪/**/*.c` 的注释表。
...
| 24  | 代码注释里写 Markdown 表格却**不对齐**（编辑器不渲染注释 ⇒ 屏幕上就是一堆错位竖线）；或表内混用 `µs`/`≈`/`⇒` 等**宽窄歧义**字符 ⇒ 怎么补空格都对不齐                                                                                                                             | 按 **§三 第 6 条**：中文/全角计 **2 列**、逐行显示宽度必须相等、分隔行横线补满列宽；表内单位写 `us`；批量改完跑"逐行宽度校验"（本次即用它定位并修正了 6 个文件）                                                                             |
...
| 表格对齐自检                   | `powershell -File Tests/check_tables.ps1 [-Verbose] [-Roots Code,Docs]`（宽度口径：ASCII = 1 列 / 中文·全角 = 2 列） |
