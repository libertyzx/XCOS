...
> **版本**：本文档描述 **V2.1.0** 的完整实现；**V2.1.1 是补丁级修订**（长跑加固 + 配置收敛 + 文档契约，见 `Docs/CHANGELOG.md`「T2026/09/17-2」与 `Docs/问题跟踪/问题清单/20260917_*`）——本文已同步其中的实现差异（`PrevTick` 每轮更新、空闲 Tick 锁内取值、`pNotifyData` 加 `volatile`）。
...
/* 滴答计数数据类型(固定 32 位无符号, 不可配置; 定义在 Internal/XC_TypeInternal.h) */
...
1. **Tick 溢出处理**（`PrevTick > Tick`）：
   ...
   - 溢出表所有节点移入时间表；
   - `PrevTick` 的更新**不在本分支**：本函数**末尾每轮**统一更新（见下方注）；
...
>
> **`PrevTick` 更新纪律（2026-09-17 起，A19）**：本函数**每轮末尾**都把"当前 Tick"记为观察值（原先只在"检测到回绕"或"时间表有到点"时更新）。
> 原因是回绕判定 `PrevTick > Tick` 只依赖"上次观察到的 Tick"：若长期不更新（例如**启动后从未有定时到点**的应用，
> 期间却有一笔跨回绕的延时进了溢出表），回绕真正发生时判定不成立 ⇒ **溢出表整表搬移被跳过** ⇒ 该任务永不唤醒
> （最长再等一个完整回绕周期，若期间仍无到点则永久）。每轮更新不改变判定语义，只让它更及时。
...
| 当前 Tick    | **同一锁内**取 Tick 快照（2026-09-17 起，A20）                | 原写法在**进锁之前**读 Tick ⇒ 与表内取值之间会被中断推进 ⇒"距唤醒还有多少"被**高估** ⇒ 提示值偏大（应用可能睡过头）；与表内取值放同一锁域 ⇒ 一次一致观察                                                                                 |
...
| `TCB.pNotifyData`                                                                    | ✅ **volatile**                 | 生产者 `SendNotify` / `XC_Task_UpdateNotifyData`                                                | **SPSC（发布顺序）**          | 字对齐写原子；**加 `volatile` 后可保证"先写数据 → 再置标志"不被编译器重排** ⇒ 消费侧见到标志就读到新数据；多生产者 ⇒"最后写入胜出"   |
