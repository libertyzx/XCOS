 ...
 *  在任务注册,移除,复位,添加时调用
 */
static void XCTask_BasicInit(XC_TaskHandle_t phTCB)
{
    _COR_Init(phTCB->BP);                     // 初始化断点
    phTCB->TaskWakeupTick = ~0;               // 任务下个唤醒的时间(初始设置最大)
 ...
 */
static void XCTask_AddInternal(XC_TaskHandle_t phTCB)
{
    XCTask_BasicInit(phTCB);                                                 // 基础初始化
    XCSch_Lock(phTCB->phXCOS);                                               // 锁
    XCList_InsertNodeAfter(phTCB->phXCOS->pPrevReadyNode, &phTCB->ListNode); // 插入就续表
...
void XCTask_HandleSuspend(XC_TaskHandle_t phTCB)
{
    ...
    XCSch_Lock(phTCB->phXCOS);           // 锁
    XCTask_MoveTaskToBlockedList(phTCB); // 将任务移动到阻塞表
    XCSch_Unlock(phTCB->phXCOS);         // 解锁
}
 ...
 */
static void XCTask_HandleBlocking(XC_TaskHandle_t phTCB, uint32_t TickCount)
{
    XC_Tick_t     Tick;
    XCListNode_t* pIterator;
    XCListNode_t* pList;

    if(TickCount != 0) {                   // 有延时或超时时间("TickCount"!=0)
         ...
         */
        pIterator = XCList_GetListStartNode(pList); // 获取链表的首节点
         ...
         *  即为,新节点插入在迭代器的上个节点;
         */
        XCList_MoveNodeAfter(pIterator->pPrev, &phTCB->ListNode);
    }
    else {                                   // 无时间,阻塞处理
        XCSch_Lock(phTCB->phXCOS);           // 锁
...
XC_Retuen_t XCTask_Resume(XC_TaskHandle_t phTCB)
{
    ...
    XCSch_Lock(phTCB->phXCOS);         // 锁
    XCTask_MoveTaskToReadyList(phTCB); // 将任务移动到就绪表
    XCSch_Unlock(phTCB->phXCOS);       // 解锁
    phTCB->TaskState = XC_TASK_READY;  // 任务状态:就绪
    return (XC_OK);
}
...
XC_Retuen_t XCTask_SendNotify(XC_TaskHandle_t phTCB, void* pNotifyData)
{
    ...
    /** 调度没有运行 */
    XCSch_Lock(phTCB->phXCOS);             // 锁
    XCTask_MoveTaskToReadyList(phTCB);     // 移动到就绪表
    XCSch_Unlock(phTCB->phXCOS);           // 解锁
    phTCB->NotifyState = XC_NOTIFY_WAKEUP; // 通知状态:通知唤醒
    phTCB->TaskState   = XC_TASK_READY;    // 任务状态:就绪
    return (XC_OK);
