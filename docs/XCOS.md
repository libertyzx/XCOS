...
| 宏名      | 值    | 所在文件  | 说明                          |
| ---       | ---   | ---       | ---                           |
| __XCOS__  | 200   | XCOS.h    | 用于标记XCOS框架,同时表明版本 |

## 全局变量

| 名称                      | 32位下占用字节    | 所在文件  | 说明                      |
| ---                       | ---               | ---       | ---                       |
| _XC_CreateSysTickCount    | 4                 | XC_Cnf.h  | [用户]定义一个Tick计数值  |
...
| XCOS_t        | 52                | XC_Sch.h  | 定义一个XCOS框架的句柄            |
...
| XC_GetWakeTimeout     | 宏    |  [用户][协程]获取唤醒超时                             | XC_Task.h |
...
- 这个表说明了所有函数被中断的等级
    ...
    - [3]用户:     正常用户大循环中调用;
    - [2]中断:     能在中断中任意位置打断"[4]协程层"和"[3]用户"的函数;
    - [1]任意:     任何位置可调用;
...
| ---                   | ---                           | ---               | ---                   | ---                   | ---                                                   |
|                       |                               |                   |                       | XC_AccSysTickCount    | [用户]累加滴答时间计数                                |
| XC_Enter              |                               |                   |                       |                       | [用户][协程]进入                                      |
| XC_Leave              |                               |                   |                       |                       | [用户][协程]离开                                      |
| XC_Yield              |                               |                   |                       |                       | [用户][协程]让出控制                                  |
| XC_Reset              |                               |                   |                       |                       | [用户][协程]任务复位                                  |
...
| XC_GetWakeTimeout     |                               |                   |                       |                       | [用户][协程]获取唤醒超时                              |
