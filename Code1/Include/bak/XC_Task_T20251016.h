 ...
 *  类型占字节数(32bit): 12+4*6+3=39Byte,补齐占:40Byte
 */
typedef struct _XCTCB_t {
     ...
     *  "XCState_t"类型数据;
     */
    uint8_t TaskState; // 任务状态(XCState_t)
 ...
 * @details     当前框架中有多少任务;
 */
uint8_t XC_GetTaskNum(XCOS_t* phXCOS);
...
/**
 * @brief       [用户][协程]协程块内获取任务注册时传递的参数(uint32_t)
 * @return      uint32_t    返回32位无符号整形;
 * @details
 *  **必须在协程块中使用;** \n
 *  用来获取参数;
 *  > 注意: 因为协程内上下文切换局部变量是不保存的,
 *  > 所以若是要使用传递的参数需要再"XC_Enter"前将参数赋值给变量;
 */
#define XC_GetParamUint() (_phXCTCB->Param)
...
/**
 * @brief       [用户][协程]获取通知的数据(uint32_t)
 * @return      uint32_t    返回通知数据
 * @details
 *  **必须在协程块中使用;** \n
 *  被通知唤醒后获取通知传递的数据;
 */
#define XC_GetNotifyDataUint()       (_phXCTCB->NotifyData)
...
/**
 * @brief       [用户]获取任务注册时传递的参数(uint32_t)
 * @return      uint32_t    返回32位无符号整形;
 * @details
 *  **任意位置可调用;** \n
 *  用来获取任务参数;
 ...
 *  > 所以若是要使用传递的参数需要再"XC_Enter"前将参数赋值给变量;
 */
#define XC_GetTaskParamUint(_phTCB)  ((_phTCB)->Param)
...
/**
 * @brief       [用户]更新通知数据(uint32_t)
 * @param[in]   _phTCB      [XCTCB_t*]任务控制块
 * @param[in]   _NotifyData [uint32_t]通知数据
 * @details
 *  **任意位置可调用;** \n
 *  在不使用任务通知的时候,可以用通知数据来传递数据; \n
 *  此函数用于更新(写)通知数据;
 */
#define XC_UpdateNotifyDataUint(_phTCB, _NotifyData) \
    {                                                \
        (_phTCB)->pNotifyData = (_NotifyData);       \
    }
 ...
 *  此函数用于读取通知数据;
 */
#define XC_ReadNotifyDataUint(_phTCB)          ((_phTCB)->NotifyData)

/**
 * @brief       [用户]发送通知(uint32_t)
 * @param[in]   phTCB       任务控制块
 * @param[in]   pNotifyData 通知传递的数据(uint32_t)类型
 * @return      int32_t
 * @retval      _XC_R_OK :          通知成功
 * @retval      _XC_R_Continue :    任务已经唤醒(已在就绪表)
 * @retval      _XC_R_Fail :        任务被挂起
