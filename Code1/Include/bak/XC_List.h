...
/**
 * @brief       [内部]连接2个节点
 * @param[in]   pPrevNode   上个节点
 * @param[in]   pNextNode       下个节点
 * @details     调用此函数后会删除参数中两个节点间的所有节点
 */
void XCList_LinkNode(XCListNode_t* pPrevNode, XCListNode_t* pNextNode);
...
/**
 * @brief       [内部]将节点插入链表头(链表的开始)
 * @param[in]   pList       [XCListNode_t*]需插入的链表
 * @param[in]   pNewNode    [XCListNode_t*]新节点
 * @details     插入根节点头之后;
 */
#define XCList_InsertFront(_pList, _pNewNode) XCList_InsertNodeAfter((_pList), (_pNewNode))

/**
 * @brief       [内部]将节点插入链表尾(链表结尾)
 * @param[in]   pList       [XCListNode_t*]需插入的链表
 * @param[in]   pNewNode    [XCListNode_t*]新节点
 * @details     插入根节点头之前;
 */
#define XCList_InsertBack(_pList, _pNewNode)  XCList_InsertNodeBefore((_pList), (_pNewNode))
