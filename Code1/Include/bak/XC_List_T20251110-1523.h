...
typedef struct _XCListRoot_t {
    XCListNode_t RootNode; // 根节点,最后/最初的节点
} XCListRoot_t;
...
/**
 * @brief       [内部]将节点插入链表的开始
 * @param[in]   pList       [XCListRoot_t*]需插入的链表
 * @param[in]   pNewNode    [XCListNode_t*]新节点
 * @details     插入RootNode头;
 */
#define XCList_InsertStart(_pList, _pNewNode) XCList_InsertNodeNext((XCListNode_t*)&(_pList)->RootNode, (_pNewNode))
 ...
 * @details     插入RootNode尾;
 */
#define XCList_InsertEnd(_pList, _pNewNode)   XCList_InsertNodePrevious((XCListNode_t*)&(_pList)->RootNode, (_pNewNode))
