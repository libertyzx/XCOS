...
/************************************************|
 * 描述:    比较当前Tick
 * 宏名:    XCTime_CompareTickt
 * 形参[I]: _Lc     //上个记录的Tick
 * 返回:    boor    //1:时间到达;0:时间没有到达;
 * 说明:    系统Tick和"_Lc"直接比较,判断是否到达"_Lc";
 * 例程:    无
 ************************************************/
#define XCTime_CompareTickt(_Lc)        (_XC_SysTickCount >= (_Lc))
 ...
 * 例程:    无
 ************************************************/
#define XCTime_Delay(_Lc, _Dc)                  \
    {                                           \
        (_Lc) = _Time_GetTick();                \
        while(!_Time_CompareTick((_Lc), (_Dc))) \
            ;                                   \
    }
 ...
 * 例程:    无
 ************************************************/
#define XCTime_tDelay(_tC, _n)           \
    {                                    \
        XCTime_tUpdateTick((_tC), (_n)); \
        while(!XCTime_tCompareTick(_tC)) \
            ;                            \
    }
