...
- 功能详细修改点
    | 名称                      | 类型      | 所在文件          | 功能                                              | 操作 | 操作原因                                       |
    | ---                       | ---       | ---               | ---                                               | --   | ---                                            |
    | XC_GetParamUint           | 函数宏    | XC_Task.h         | [用户]协程块内获取任务注册时传递的参数(uint32_t)  | 删除 | 基础为指针传递的,没有必要在做一层整型数据传递  |
    | XC_GetNotifyDataUint      | 函数宏    | XC_Task.h         | 获取通知的数据(uint32_t)                      | 基础为指针传递的,没有必要在做一层整型数据传递     |
    | XC_GetTaskParamUint       | 函数宏    | XC_Task.h         | 获取任务注册时传递的参数(uint32_t)            | 基础为指针传递的,没有必要在做一层整型数据传递     |
    | XC_UpdateNotifyDataUint   | 函数宏    | XC_Task.h         | 更新通知数据(uint32_t)                        | 基础为指针传递的,没有必要在做一层整型数据传递     |
    | XC_ReadNotifyDataUint     | 函数宏    | XC_Task.h         | 读取通知数据(uint32_t)                        | 基础为指针传递的,没有必要在做一层整型数据传递     |
    | XC_SendNotifyUint         | 函数宏    | XC_Task.h         | 发送通知(uint32_t)                            | 基础为指针传递的,没有必要在做一层整型数据传递     |
    | XC_TaskReset              | 函数宏    | XC_Task.h         | 复位当前任务                                  | 已有"XC_Reset",不在做兼容                         |
    | XC_Delay_min              | 函数宏    | XC_Task.h         | 按min延时                                     | 延时最高到s,后面自行转换                          |
    | XC_Delay_h                | 函数宏    | XC_Task.h         | 按小时延时                                    | 延时最高到s,后面自行转换                          |
    | XC_Delay_day              | 函数宏    | XC_Task.h         | 按天延时                                      | 延时最高到s,后面自行转换                          |
    | XCTime_CompareTick_min    | 函数宏    | XC_TimeCount.h    | 按min比较时间                                 | 时间处理最高到s,后面自行转换                      |
    | XCTime_CompareTick_h      | 函数宏    | XC_TimeCount.h    | 按h比较时间                                   | 时间处理最高到s,后面自行转换                      |
    | XCTime_CompareTick_Day    | 函数宏    | XC_TimeCount.h    | 按day比较时间                                 | 时间处理最高到s,后面自行转换                      |
    | XCTime_Delay_s            | 函数宏    | XC_TimeCount.h    | 按s死循环                                     | 因为看门狗,死循环最大单位为ms                     |
    | XCTime_Delay_min          | 函数宏    | XC_TimeCount.h    | 按min死循环                                   | 因为看门狗,死循环最大单位为ms                     |
    | XCTime_Delay_h            | 函数宏    | XC_TimeCount.h    | 按h死循环                                     | 因为看门狗,死循环最大单位为ms                     |
    | XCTime_Delay_day          | 函数宏    | XC_TimeCount.h    | 按day死循环                                   | 因为看门狗,死循环最大单位为ms                     |
    | XCTime_GetTick_ms         | 函数宏    | XC_TimeCount.h    | 获取Tick+min后的Tick                          | 和"XCTime_CompareTickt"一起处理有安全隐患,故删除  |
    | XCTime_GetTick_s          | 函数宏    | XC_TimeCount.h    | 获取Tick+min后的Tick                          | 和"XCTime_CompareTickt"一起处理有安全隐患,故删除  |
    | XCTime_GetTick_min        | 函数宏    | XC_TimeCount.h    | 获取Tick+min后的Tick                          | 和"XCTime_CompareTickt"一起处理有安全隐患,故删除  |
    | XCTime_GetTick_h          | 函数宏    | XC_TimeCount.h    | 获取Tick+h后的Tick                            | 和"XCTime_CompareTickt"一起处理有安全隐患,故删除  |
    | XCTime_GetTick_day        | 函数宏    | XC_TimeCount.h    | 获取Tick+day后的Tick                          | 和"XCTime_CompareTickt"一起处理有安全隐患,故删除  |
    | XCTime_CompareTickt       | 函数宏    | XC_TimeCount.h    | 比较当前Tick判断时间计数是否到达              | 和"XCTime_GetTick_*"一起处理有安全隐患,故删除     |
    | XCTime_tDelay             | 函数宏    | XC_TimeCount.h    | 按Tick死循环延时(扩展)                        | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tDelay_ms          | 函数宏    | XC_TimeCount.h    | 按ms死循环延时(扩展)                          | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tDelay_s           | 函数宏    | XC_TimeCount.h    | 按s死循环延时(扩展)                           | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tDelay_min         | 函数宏    | XC_TimeCount.h    | 按min死循环延时(扩展)                         | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tDelay_h           | 函数宏    | XC_TimeCount.h    | 按h死循环延时(扩展)                           | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tDelay_day         | 函数宏    | XC_TimeCount.h    | 按day死循环延时(扩展)                         | 已有"XCTime_BlockDelay"和"XCTime_BlockDelay_ms"   |
    | XCTime_tUpdateTick_min    | 函数宏    | XC_TimeCount.h    | 更新需要延时比较的时间(min)                   | 时间处理最高到s,后面自行转换                      |
    | XCTime_tUpdateTick_h      | 函数宏    | XC_TimeCount.h    | 更新需要延时比较的时间(h)                     | 时间处理最高到s,后面自行转换                      |
    | XCTime_tUpdateTick_Day    | 函数宏    | XC_TimeCount.h    | 更新需要延时比较的时间(Day)                   | 时间处理最高到s,后面自行转换                      |
    | XCTime_tGetTickCount      | 函数宏    | XC_TimeCount.h    | 获取类型中的Tick计数值                        | 不常用,删除                                       |
    | XCTime_tSetTickCount      | 函数宏    | XC_TimeCount.h    | 设置类型中的Tick计数值                        | 不常用,删除                                       |
    | XCTime_tClrTickCount      | 函数宏    | XC_TimeCount.h    | 清除类型中的Tick计数值                        | 不常用,删除                                       |
    | XCTime_tGetWaitCount      | 函数宏    | XC_TimeCount.h    | 获取类型中的等待计数值                        | 不常用,删除                                       |
    | XCTime_tSetWaitCount      | 函数宏    | XC_TimeCount.h    | 设置类型中的等待计数值                        | 不常用,删除                                       |
    | XCTime_tClrWaitCount      | 函数宏    | XC_TimeCount.h    | 清除类型中的等待计数值                        | 不常用,删除                                       |
    | _Time_\*\*2\*\*           | 函数宏    | XC_TimeCount.h    | 从ms到year的各类时间转换的比例                | 不使用,删除   |
    | _Time_min2Tick            | 函数宏    | XC_TimeCount.h    | 时间转Tick                                    | 不使用,删除   |
    | _Time_h2Tick              | 函数宏    | XC_TimeCount.h    | 时间转Tick                                    | 不使用,删除   |
    | _Time_day2Tick            | 函数宏    | XC_TimeCount.h    | 时间转Tick                                    | 不使用,删除   |
    | _Time_Tick2min            | 函数宏    | XC_TimeCount.h    | Tick转时间                                    | 不使用,删除   |
    | _Time_Tick2h              | 函数宏    | XC_TimeCount.h    | Tick转时间                                    | 不使用,删除   |
    | _Time_Tick2day            | 函数宏    | XC_TimeCount.h    | Tick转时间                                    | 不使用,删除   |
...
- 函数修改
    ...
    | XCSch_GetTaskNum                  | XC_Sch.c          | XC_GetTaskNum                     | XC_Task.c     | 修改函数名称                          |
...
- 将任务相关处理函数从"XC_Sch.c"移动到"XC_Task.c"并改名:
    | "XC_Sch.c"文件的函数              | "XC_Task.c"文件的函数             | 说明                              |
    | ---                               | ---                               | ---                               |
    | XCSch_TaskReg                     | XC_TaskReg                        | [用户]任务注册                    |
    | XCSch_TaskRemove                  | XC_TaskRemove                     | [用户]任务移除                    |
    | XCSch_GetTaskNum                  | XC_GetTaskNum                     | [用户]获取任务数                  |
    | XCSch_SetTaskEntryPoint           | XC_SetTaskEntryPoint              | [用户]分步添加任务-设置任务入口   |
    | XCSch_AddTask                     | XC_AddTask                        | [用户]分步添加任务-添加任务       |
    | XCSch_ListNodeRemove              | XC_ListNodeRemove                 | [内部]链表节点移除                |
    | XCSch_ListNodeInsertIndexPrevious | XC_ListNodeInsertIndexPrevious    | [内部]将节点插入索引前            |
    | XCSch_ListNodeInsertAsc           | XC_ListNodeInsertAsc              | [内部]升序排列的插入节点          |
- 删除函数(函数宏):
    | 函数                      | 文件              | 功能                                          | 删除原因                                          |
    | ---                       | ---               | ---                                           | ---                                               |
    | XC_GetParamUint           | XC_Task.h         | 协程块内获取任务注册时传递的参数(uint32_t)    | 基础为指针传递的,没有必要在做一层整型数据传递     |
